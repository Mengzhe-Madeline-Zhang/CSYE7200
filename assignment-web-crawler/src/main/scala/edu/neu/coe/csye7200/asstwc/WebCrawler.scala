...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] =
      for{n <- ns\\"a"map(_\"@href")map(_.toString)} yield createURL(Some(u), n)
     // TO BE IMPLEMENTED
    ...
    // 9 points.
       for(url <- getURLContent(u); g<-MonadOps.asFuture(getLinks(url))) yield g// TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
     val mwu =for(u<-us2; wu = wget(u)) yield MonadOps.sequence(wu)
     Future.sequence(mwu)
  }
